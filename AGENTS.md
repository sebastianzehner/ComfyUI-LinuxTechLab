...
3. **Frontend Component Design:** Create and extend Vue.js components in `web/js/`. Ensure that `__init__.py` exports `WEB_DIRECTORY = "./web"` to register the frontend directory with ComfyUI. Node documentation is placed in `web/docs/<node_id>/en.md` with node-specific assets in the same directory. Image paths in Markdown are relative to the `docs/` directory, not to the Markdown file itself — use `<node_id>/image.webp` as path.
...
- **UI Information:** Always include a `description` in the `io.Schema` for all nodes to provide clear information in the ComfyUI interface.
